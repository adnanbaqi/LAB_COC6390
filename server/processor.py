...
class StreamProcessor:
    ...
    def _draw_detections(self, frame: np.ndarray, detections: List[Detection]) -> np.ndarray:
        ...
        for det in detections:
            ...
            # 1. Handle our new vehicle prefixes for standard drawing
            if det.label.startswith("vehicle_"):
                display_label = det.label.replace("vehicle_", "")
                color = (255, 255, 0) # Cyan for normal traffic
                
                cv2.rectangle(annotated, (x1, y1), (x2, y2), color, 2)
                cv2.putText(annotated, f"{display_label} {det.confidence:.0%}", (x1, y1 - 6),
            ...
            if det.label in ["fire", "smoke"]:
                ...
                color = (0, 0, 255)
            elif det.label == "tripling":
                color = (255, 0, 255)  # Magenta for tripling
            else:
                color = (0, 200, 50)
    ...
    def _persist(self, raw_frame: np.ndarray, det: Detection):
        ...
        # Skip DB logging and snapshots for standard traffic flow
        if det.label.startswith("vehicle_"):
            return 
        ...
        try:
            if det.label == "illegal_parking":
                ...
                send_sms_alert(f"TRIPLING ALERT: Cam {self.camera_id}\nPlate: {plate_number}\nCount: {det.meta.get('person_count')}")
                send_rto_email(plate_number, self.camera_id, 0.0, snapshot_path)
